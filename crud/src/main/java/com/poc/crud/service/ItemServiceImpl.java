...
public class ItemServiceImpl implements  ItemService{
    ...
    public void processAndSaveData(MultipartFile file)  {
        ...
        try {
            listOfItems = csvTOJavaObject(file.getInputStream());
            for(Item item : listOfItems){
                itemDao.saveItem(item);
            }
        } catch (IOException e) {
            throw new RuntimeException("Error while getting input stream");
        }
    }
    ...
    public Date parseStringToDate(String inputDate) {
        ...
        try {
            ...
            throw new RuntimeException("Date format should be dd/MM/yyyy");
        }
        return parsedDate;
    }
